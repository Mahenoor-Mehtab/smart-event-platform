...
const CreateEvent = ()=>{
   ...
   const { register , handleSubmit, watch , setValue , control, formState:{ errors } }= useForm({
    ...
    // watch is used to update the ui during dynamic changes 
        ...
        <div style={{backgroundColor: themeColor}} className='min-h-screen transition-colors duration-300 px-6 py-8 -mt-6 md:-mt-16 md:-mr-26 lg:rounded-md'>
            ...
            <div className='max-w-6xl mx-auto flex flex-col gap-5 md:flex-row justify-between mb-10'>
                <div>
                    ...
                    {
                        !hasPro && (
                            <p className='text-sm text-muted-foreground mt-2'>Free: {currentUser?.freeEventCreated || 0}/1 events created</p>
                        )
                    }
                </div>
                {/* Ai Event Creator */}
...
Paid
          ...
          </div>
          {
            ticketType === "paid" && (
              <Input
              type="number"
              placeholder="Ticket price ₹" {...register("ticketPrice", {valueAsNumber: true })}
              />
            )
          }
          </div>
